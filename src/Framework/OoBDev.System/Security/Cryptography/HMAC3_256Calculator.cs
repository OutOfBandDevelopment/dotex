...
/// <summary>
/// Calculates HMAC-SHA3-256 hashes for message authentication.
/// </summary>
public class HMAC3_256Calculator : IHMACCalculator
{
    /// <summary>
    /// Calculates the HMAC-SHA3-256 hash of a message using the provided secret key.
    /// </summary>
    /// <param name="secret">The secret key for HMAC calculation.</param>
    /// <param name="message">The message to hash.</param>
    /// <returns>The HMAC-SHA3-256 hash as a byte span.</returns>
    public ReadOnlySpan<byte> Calculate(string secret, string message)
    {
        var secretBytes = Encoding.UTF8.GetBytes(secret);
        var uriBytes = Encoding.UTF8.GetBytes(message);
        using var hmac = new HMACSHA3_256(secretBytes);
    ...
    /// <summary>
    /// Calculates the HMAC-SHA3-256 hash and encodes it as a Base64 string in one operation.
    /// </summary>
    /// <param name="secret">The secret key for HMAC calculation.</param>
    /// <param name="message">The message to hash.</param>
    /// <returns>The Base64-encoded HMAC-SHA3-256 hash.</returns>
